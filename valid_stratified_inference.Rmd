...
$$ C_k = \log(\alpha) + \log(\beta) + \log(\alpha + Y_k) + \log(\beta + n_k - Y_k) - \log(\alpha + \beta) - \log(\alpha + \beta + n_k) $$
...
for(j in 1:length(true_mean)){
  ...
  for(i in 1:length(n_grid)){
    ...
    replicates_eb_unstrat <-  replicate(n = B, get_stratified_pvalue(population = binary_population, strata = rep(1,2000), mu_0 = 0.5, n = n_grid[i], method = "empirical_bernstein"))
    replicates_eb_strat <- replicate(n = B, get_stratified_pvalue(population = binary_population, strata = c(rep(1,1000), rep(2, 1000)), mu_0 = 0.5, n = c(n_grid[i]/2,n_grid[i]/2), method = "empirical_bernstein"))
...
power_frame_3_point <- data.frame("power_eb_stratified" = power_eb_strat, "power_eb_unstratified" = power_eb_unstrat, "power_ppm_unstratified" = power_ppm_unstrat, "sample_size" = n_grid) %>%
  pivot_longer(cols = starts_with("power"), names_to = "design", values_to = "power", names_prefix = "power_") %>%
  separate(design, into = c("design","true_mean"), sep = ".true_mean_")
  #separate(design, into = c("martingale", "design"), sep = "_")
...
hist(highly_stratified_population, main = "Mixture distribution of 5 Betas")


B <- 300
n_grid <- round(10^seq(2, 4, length.out = 15))
mu_0_grid <- c(.08, .09, .095, .1) 
