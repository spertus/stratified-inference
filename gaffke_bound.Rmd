...
This is as-yet unproven, but the empirical results are encouraging. It is clearly valid for binary populations given the equivalence to Clopper-Pearson, but the level over $\mathcal{F}_{[0,1]}$ is unproven. There is work being done by [My Phan and colleagues](https://arxiv.org/abs/2106.03163) at UMass Amherst CS. I hope to help with this too.
...
Gaffke_CI <- function(x, alpha = .05, B = 200, side = "upper"){
  n <- length(x)
  if(side == "lower"){
    x <- 1 - x
  }
...
get_Gaffke_CI <- function(x){
  c(Gaffke_CI(x = x, alpha = .025, side = "lower"), Gaffke_CI(x = x, alpha = .025, side = "upper"))
}
